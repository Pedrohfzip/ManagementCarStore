...
const highlights = [
  {
    ...
    colorClass: "bg-spec-blue/10 text-spec-blue text-white",
  },
  {
    icon: ShieldCheck,
    title: "Garantia",
    desc: "Veículo revisado",
    colorClass: "bg-spec-green/10 text-spec-green text-white",
  },
  {
    icon: ArrowLeftRight,
    title: "Aceita Troca",
    desc: "Avaliamos seu usado",
    colorClass: "bg-spec-amber/10 text-spec-amber text-white",
  },
];

const CarHighlights = () => {
  return (
    <div className="text-white grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
      {highlights.map((h, i) => {
        const Icon = h.icon;
        return (
          <motion.div
            ...
            className="text-white bg-card rounded-xl shadow-sm border border-border p-5 text-center hover:shadow-md transition-shadow"
          >
            <div
